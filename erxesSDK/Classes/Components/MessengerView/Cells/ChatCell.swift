...
//        options[NSAttributedString.Key.font] = UIFont.systemFont(ofSize: 13)
        ...
        textView.backgroundColor = UIColor(hexString: uiOptions?.color ?? defaultColorCode)
        edgeView.backgroundColor = UIColor(hexString: uiOptions?.color ?? defaultColorCode)
    ...
    override func layoutView() {
        ...
        self.contentView.bringSubview(toFront: textView)
    }
}
