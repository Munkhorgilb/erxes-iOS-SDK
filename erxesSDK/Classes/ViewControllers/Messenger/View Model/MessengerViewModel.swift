...
class MessengerViewModel {
    ...
    func conversationDetail(conversationId: String?) {
        guard let id = conversationId else {
            // Handle the case where conversationId is nil
        ...
        self.service.conversationDetail(conversationId: conversationId, success: { (data) in
            ...
            if let isOnline = data.isOnline {
                self.didSetIsOnline?(isOnline)
            } else {
                // Handle the case where data.isOnline is nil
                print("data.isOnline is nil")
            }
        }) { (error) in
